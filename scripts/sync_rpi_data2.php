            ...
            if ($stmt=$mysqli->prepare("SELECT password FROM webuser WHERE pname = ? LIMIT 1")) {
                ...
                if ($stmt->num_rows == 1) {		//if user exists, verify the password
                    if (password_verify($in_password, $db_password)) {
			            $stmt->close();
                        if ($stmt = $mysqli->prepare("UPDATE devices set status=? where devID = 'SW1'")) { //update SW1
                            $stmt->bind_param('i', $in_SW1); $stmt->execute();
			            } else {$error=1;} 
			            $stmt->close();
                        if ($stmt = $mysqli->prepare("UPDATE devices set status=? where devID = 'SW2'")) { //update SW2
                            $stmt->bind_param('i', $in_SW2); $stmt->execute();
			            } else {$error=2;} 
                        $stmt->close();
                        if ($stmt = $mysqli->prepare("UPDATE devices set status=? where devID = 'SW3'")) { //update SW3
